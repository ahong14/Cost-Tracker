...
const UserResolvers = {
    Query: {
        loginUser(_, { email, password }) {
            const loginUserAPI = async (email, password) => {
                try {
                    const loginResult = await axios.post("http://localhost:8080/api/auth/login", {
                        email: email,
                        password: password
                    });
    ...
    Mutation: {
        createUser(_, { email, password, first_name, last_name }) {
            const signupUserAPI = async (email, password, first_name, last_name) => {
                try {
                    const signupResult = await axios.post("http://localhost:8080/api/auth/signup", {
                        email: email,
                        password: password,
